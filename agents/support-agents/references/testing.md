...
- **Directive-heavy and legal-threat openers derail the simulated user.** An opener that is mostly imperatives at the company ("Remove my data immediately. I will report you to the authorities.") gives the persona LLM nothing conversational to *be* — some runs it refuses to deliver it verbatim, fabricates a closing line, and the conversation dies with zero agent turns. That's a harness artifact, not an agent result: don't grade it, don't "fix" the agent for it. Replay such tickets as next-reply tests (fixed history, only the reply graded), or neutralize the opener's framing while preserving the demands.
- **A body-less email ticket is not an opener.** Email tickets sometimes carry the whole ask in the *subject* and only a signature in the body ("Sent from my iPhone" and its localizations). A persona built from the body alone replays the signature and improvises an unrelated scenario. When the body is uninformative, fold the subject into the opening message.
