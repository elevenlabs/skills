You are tasked with updating ElevenLabs agent skills based on a merged weekly changelog in [elevenlabs-dx](https://github.com/elevenlabs/elevenlabs-dx).

For Cursor Cloud Automations, follow `.cursor/skills/update-skills-from-changelog/SKILL.md` for trigger handling, changelog fetching, relevance filtering, and pull request creation. This command contains the detailed editing, verification, and reporting rules used by that workflow.
...
2. Apply the relevance filter and identify affected skills
...
6. Create a branch, commit changes if needed, and open a pull request in `elevenlabs/skills`
...
If no skills are affected, stop successfully without opening a pull request.

For each relevant item, note the affected skill and the **affected area** (for example **model table**, **code examples**, **LLM provider table**, **tools section**, **CLI section**).

## Step 3: Read current skill files

For each affected skill listed in the changelog analysis, read:
...
Before making any edits, fetch and read the actual source material. The changelog tells you *what changed*; API/reference docs tell you *what to document as the current state*. You must verify every detail before writing documentation.

1. **Fetch the changelog page** linked above (for example `https://elevenlabs.io/docs/changelog#YYYY-MM-DDT00:00:00.000Z`). Use it to identify deltas and affected surfaces, not as final wording for skill files.
...
3. **For each relevant changelog item**, find the corresponding section in the API reference. Note the exact field names, types, nesting, and allowed values for the final docs.
4. If a feature appears in the changelog but the API reference does not provide enough schema detail, do not create field tables or code examples for it. Put it under "Needs Manual Authoring" in the final report.
...
Before making any edits, run this decision gate for each relevant changelog item:
...
For each relevant item that passes Step 3.6, identify the **affected area** and apply the change to the correct file and section using current-state, present-tense wording.
...
- **Never invent field names, types, or schemas.** Every field you document must be verified against API reference documentation, not inferred from the changelog. If the changelog mentions a new `sanitize` field but you cannot find its exact type and parent object in the API reference, do not document it.
...
- **Treat the changelog as a discovery input, not skill-file prose.** It tells you *which* things changed; API/reference docs tell you the current behavior to document. Never use changelog phrasing directly in skill content.
...
- **Skill files must be evergreen.** Never mention changelog, issue, PR, release date, or temporal phrases like "added in", "introduced in", "as of YYYY-MM-DD", or "now supports" inside `SKILL.md` or `references/*.md`.
...
**Low risk** (changelog + API reference verification is usually sufficient):
...
- Make minimal, targeted changes. Do not rewrite sections that aren't implicated by the changelog.
...
- If the changelog mentions an SDK version bump but no method signature changes, update any version-specific comments but don't change code examples.
...
5. No edited `SKILL.md` or `references/*.md` line references a changelog, issue, PR, or date as part of feature documentation.
...
12. Every relevant changelog item is accounted for via one of: (a) natural-home docs update, (b) justified new section, (c) "No Skill Change Needed", or (d) "Needs Manual Authoring" in the final report.
...
## Step 5: Create branch, commit, and open pull request

Skip if an open PR already exists for this changelog date:
...
Write the pull request body to `/tmp/skills-update-report.md` using the report template below, then open the pull request:
...
If there are no file changes after analysis, do not push or open a PR. Return the report only.

When run via Cursor CLI headless mode, return the completed markdown report in the final response. When run as a Cursor Cloud Automation with write access, opening the pull request is required unless no skill files changed.

The report / PR body should follow this structure:
...
List any items from the changelog that were NOT applied because the schema could not be
...
- Do not modify skills that are not implicated by the changelog.
- Do not update the `openclaw/` directory — that is community-maintained.
- Do not change the YAML frontmatter in SKILL.md files unless the changelog specifically calls for it (e.g., description change).
- If a changelog item cannot be verified against the API reference (or another canonical documentation page), do NOT add it to the skill files. Instead, list it in the final report under "Needs Manual Authoring" with what the changelog says and why you couldn't verify it. A wrong code example is worse than a missing one.
- If a changelog item is verified but does not fit naturally into existing skill structure, do NOT force a new section or sentence. List it under "No Skill Change Needed" in the final report with a short rationale.
- If the changelog describes breaking changes, add a warning in the report highlighting which examples may need migration guidance.
- Keep changelog dates and release-history wording in the report/PR context only — never in `SKILL.md` or `references/*.md`.

## Note on changelog coverage

If the changelog only provides feature names and high-level descriptions without API reference links or schema details, many items will likely end up under "Needs Manual Authoring." This is the correct outcome — it surfaces gaps for a human to fill rather than hallucinating plausible-looking documentation.
